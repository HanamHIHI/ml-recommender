...
# print(len(data_df))
# temp_data = data_df[['Unnamed: 0', 'name', 'rate', 'review']].values.tolist()
test_data = [[] for _ in list(range(len(mapper)))]
temp_data = []
...
try:
    model_state_dict = torch.load(tag + "_epochs_" + str(epochs) + "_batchsize_" + str(batch_size) +  "_dataset_"  + str(train_count+test_count) +  ".pt", map_location=device)
    try:
...
# print(np.array(mean_vectors[0][0]))
# print(np.array(mean_vectors[0]).shape)
