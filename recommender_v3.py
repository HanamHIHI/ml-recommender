...
targetText = "생명이란 무엇인가를 잘 알게 되었습니다" #상상 강의평
...
for sim in simList:
    ...
    meanSim.append(np.mean(np.array(sim[:3]+sim[len(sim)-3:])))
    stdSim.append(np.std(np.array(sim[:3]+sim[len(sim)-3:])))
...
for result in results[:10]:
    # if(result[2] >= 0.5):
    #     print(result)
    print(result[0], result[1], round(result[2], 4), round(result[3], 4))
print("\n입니다.")
print("=========================================")